...
### 1.1 问题定义：从“框得准”到“绑定是否成立”

传统 grounding 评测主要问一个问题：给定正向指称，模型能否输出与 GT 有较高 IoU 的框。这个指标不能回答两个更基础的问题：图像中是否存在该对象，以及指称中的属性/关系是否真的绑定到该对象。V-SIGHT 因此把输入从单一正查询扩展为同图像、同 GT、同目标的反事实查询对：保留正查询作为 KEEP，并只改变一个语义因素构造 REJECT 查询。这样，错误不再被混写成“定位失败”，而能区分为对象层失败与绑定层失败。

- **BOH（Basic-Object Hallucination）**：对象类别或共现对象不存在，回答了“这个对象/组合是否在图中”；
- **ROH（Relation/attribute-Object Hallucination）**：对象存在，但颜色、属性、空间关系或目标—参照物绑定不成立，回答了“这句话是否绑定到了这个实例”。

形式上，对图像 $I$、正指称 $q^+$、目标框 $b^*$，构造四类单因素负查询 $q^-_k$，其中 $k\in\{object,co\text{-}occurrence,attribute,relation\}$。判别任务要求模型输出 $KEEP/REJECT$；grounding 任务还要求输出框 $\hat b$。我们分别记录：正查询定位质量、负查询的误定位率 $FG@Neg$、假拒绝率 $FNR$，以及 BOH/ROH 分组差异。该定义把“拒绝错误表达式”和“给错误表达式画框”明确分开。

### 1.2 三个互补任务

修复版 benchmark 将同一批图像组投影到三个任务，而不是把单一 IoU 当作幻觉缓解的充分证据：

| 任务 | 输入/输出 | 测量的能力 | 关键风险 |
|---|---|---|---|
| **T1 判别 VQA** | 图像 + 查询 → KEEP/REJECT | 是否识别对象存在性和语义绑定 | ROH 查询被当作“差不多正确” |
| **T2 VQA + grounding** | 图像 + 查询 → 回答与框 | 是否在拒绝时停止发射候选框 | 负查询仍被强行定位 |
| **T4 caption 后定位** | 图像 → caption → 查询/框 | 生成描述与后续 grounding 是否一致 | caption 先引入错误，再把错误定位出来 |

每个模型在 500 个互不重复图像组上按统一角色构造 500 个正查询和 2,000 个负查询，并对 T1/T2/T4 分别保留完整记录。修复版指的是任务角色、正负语义、bbox、计数和评测协议经过统一校正；它不等价于“每条样本都是人工金标”，也不把同一模型的复核请求包装成跨模型共识。

### 1.3 11-model 结果：问题不是单一模型偶然失误

11 个 grounding 模型的 T1 结果显示，ROH 负查询误答率始终高于 BOH：ROH−BOH gap 为 **11.3–30.8pp**，其中 Qwen3-VL 为 **24.2pp**、Qwen2.5-VL 为 **21.1pp**、Orsta 为 **22.7pp**、Seg-zero 为 **30.8pp**。这说明 BOH/ROH 不是同一个难度轴上的噪声，而是可跨模型重复观察到的两种失败机制。

T2 进一步揭示“判别错误”与“定位错误”的脱钩：Seg-zero、LENS、UniVG-R1 在 ROH 负查询上的 $FG@Neg$ 分别达到 **98.9%、98.0%、99.9%**，而对应 BOH 为 **95.3%、90.4%、99.7%**。即使模型具备较高的正样本 mIoU，面对不存在的关系/属性绑定时也可能继续输出高置信候选框。

下表列出修复版 T1 的完整模型级结果；HR 是错误率，gap=ROH HR−BOH HR：

| 模型 | Balanced Acc | T1 HR | BOH HR | ROH HR | gap |
|---|---:|---:|---:|---:|---:|
| Qwen3-VL-8B | 82.8% | 23.7% | 11.6% | 35.8% | 24.2pp |
| Vision-R1 | 81.1% | 30.3% | 16.8% | 43.9% | 27.1pp |
| Qwen2.5-VL-7B | 80.9% | 22.4% | 11.8% | 32.9% | 21.1pp |
| Orsta-7B | 80.8% | 26.5% | 15.1% | 37.8% | 22.7pp |
| Visual-RFT | 80.5% | 26.6% | 13.4% | 39.7% | 26.3pp |
| Seg-R1 | 78.6% | 20.8% | 10.2% | 31.4% | 21.2pp |
| VisionReasoner | 76.8% | 42.2% | 28.3% | 56.1% | 27.8pp |
| TreeVGR | 76.6% | 45.8% | 34.2% | 57.4% | 23.2pp |
| Seg-zero | 75.9% | 44.8% | 29.4% | 60.2% | 30.8pp |
| LENS | 70.8% | 37.1% | 25.0% | 49.3% | 24.3pp |
| UniVG-R1 | 60.4% | 23.2% | 17.5% | 28.8% | 11.3pp |

因此，本文的问题不是“如何把正样本框得更准”这一单一定位问题，而是：**如何在保留可用候选框的同时，识别对象不存在、属性错误和关系绑定错误，并对不同上游模型输出进行可审计的选择性修正。**
...
当前 case 图中的部分例子存在数据质量风险，尤其是“standing person sitting on the bench”这类文本自相矛盾，以及可能存在指代不清或负例实际成立的样本。因此，旧 case 图不应直接作为论文证据。新版本只保留经过独立带图复核通过、且文本变化单一的案例，并在图注中明确：案例用于解释机制，不替代全量统计。
...
![ROH audited cases](figs/cases_ROH_audited_v2.png)

![BOH audited cases](figs/cases_BOH_audited_v2.png)
