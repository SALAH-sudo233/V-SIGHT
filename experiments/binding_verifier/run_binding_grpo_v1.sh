...
DATA="$HOME/SVD/grpo_verifier/binding_pilot_v1/grpo.jsonl"
PLUGIN="$HOME/SVD/grpo_verifier/vsight_reward_plugin_binding.py"
for p in "$MODEL" "$ADAPTER" "$DATA" "$PLUGIN"; do test -e "$p" || { echo "MISSING $p"; exit 2; }; done
mkdir -p runs/binding_grpo_v1
nohup "$HOME/.miniconda3/envs/grpo_ayb/bin/swift" rlhf \
 ...
 --max_steps 800 --learning_rate 5e-6 --temperature 1.0 --beta 0.005 \
 --warmup_ratio 0.03 --seed 42 --data_seed 42 \
 ...
 --output_dir runs/binding_grpo_v1 \
 > "$HOME/binding_grpo_v1.log" 2>&1 &
echo PID=$!
