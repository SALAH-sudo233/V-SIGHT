...
<title>V-SIGHT IoU=0 Audit</title>
<style>
...
<header><h1>V-SIGHT IoU=0 Audit</h1><span class="metric" id="progress">0 / 127 completed</span><span class="spacer"></span><label class="reviewer">Reviewer <input id="reviewer" value="reviewer_1" autocomplete="off"></label></header>
<div class="toolbar"><button id="prev" title="Previous group">&larr;</button><button id="next" title="Next group">&rarr;</button><select id="filter"><option value="pending">Pending</option><option value="all">All groups</option><option value="completed">Completed</option></select><select id="groupPicker"></select><span class="spacer"></span><span id="saveStatus" class="status"></span></div>
<main><section class="visual"><h2 class="query" id="query">Loading</h2><div class="submeta"><span id="groupId"></span><span id="target"></span><span id="distractors"></span><span id="queryStatus"></span></div>
<div class="image-tools"><label><input type="checkbox" id="showGt" checked>GT</label><label><input type="checkbox" id="showBase" checked>Baseline</label><label><input type="checkbox" id="showCand" checked>Challenger</label><label><input type="checkbox" id="showAuto">Auto labels</label><span class="spacer"></span><label>Zoom <input type="range" id="zoom" min="100" max="220" value="100" step="10"><span id="zoomValue">100%</span></label></div>
<div class="image-viewport"><div class="image-stage" id="imageStage"><img id="image" alt="Audit image"><svg id="overlay"></svg></div></div><div class="legend"><span><i class="swatch gt"></i>GT</span><span><i class="swatch base"></i>Baseline</span><span><i class="swatch cand"></i>Challenger</span></div></section>
<section class="form"><div class="task-tabs"><button id="tabT2" class="active">T2 grounding</button><button id="tabT4">T4 caption grounding</button></div>
<div class="case-meta"><div><strong id="transition"></strong>transition</div><div><strong id="ious"></strong>baseline &rarr; challenger IoU</div><div class="autoMeta hidden"><strong id="autoBase"></strong>automatic baseline class</div><div class="autoMeta hidden"><strong id="autoCand"></strong>automatic challenger class</div></div>
<div class="field"><label for="failureMode">Failure mode</label><select id="failureMode"><option value="">Select</option></select></div>
<div class="field"><label for="preferredAction">Preferred action</label><select id="preferredAction"><option value="">Select</option></select></div>
<div class="field"><label for="ambiguity">Visual ambiguity</label><select id="ambiguity"><option value="">Select</option></select></div>
<div class="field"><label>Binding evidence</label><div class="evidence" id="evidence"></div></div>
<div class="field"><label for="caseNotes">Case notes</label><textarea id="caseNotes"></textarea></div><hr class="divider">
<div class="field"><label for="querySupport">Is the positive query visually supported?</label><select id="querySupport"><option value="">Select</option><option value="supported">Supported</option><option value="unsupported">Unsupported</option><option value="ambiguous">Ambiguous</option></select></div>
<div class="field"><label for="groupNotes">Group notes</label><textarea id="groupNotes"></textarea></div>
<div class="footer-actions"><span id="savedBadge"></span><span class="spacer"></span><button id="saveDraft">Save draft</button><button id="complete" class="primary">Complete &amp; next</button></div></section></main>
<script>
...
function optionize(select,values){values.forEach(v=>{const o=document.createElement('option');o.value=v;o.textContent=v.replaceAll('_',' ');select.appendChild(o)})}
optionize($('failureMode'),FAILURE_MODES);optionize($('preferredAction'),ACTIONS);optionize($('ambiguity'),AMBIGUITIES);
EVIDENCE.forEach(v=>{const label=document.createElement('label');label.innerHTML=`<input type="checkbox" value="${v}">${v.replaceAll('_',' ')}`;$('evidence').appendChild(label)});
...
function drawBoxes(){if(!group)return;const img=$('image'),svg=$('overlay');if(!img.naturalWidth)return;svg.setAttribute('viewBox',`0 0 ${img.naturalWidth} ${img.naturalHeight}`);svg.innerHTML='';const c=group.cases.find(x=>x.task===currentTask);if($('showGt').checked)drawBox(svg,c.gt_box,'#16a34a','GT');if($('showBase').checked)drawBox(svg,c.baseline_box,'#dc2626','BASE');if($('showCand').checked)drawBox(svg,c.challenger_box,'#2563eb','CAND')}
function renderGroup(){draft=group.saved_review?JSON.parse(JSON.stringify(group.saved_review)):newDraft();if(!draft.case_reviews)draft.case_reviews=Object.fromEntries(TASKS.map(t=>[t,emptyCase()]));TASKS.forEach(t=>draft.case_reviews[t]??=emptyCase());$('query').textContent=group.query;$('groupId').textContent=group.base_sample_id;$('target').textContent=`target: ${group.target_category||'unknown'}`;$('distractors').textContent=`same-class distractors: ${group.same_category_distractors}`;$('queryStatus').textContent=`structure: ${group.expression_structure}`;$('image').src='/image/'+encodeURIComponent(group.base_sample_id);$('image').onload=drawBoxes;$('savedBadge').innerHTML=group.saved_review?(group.saved_review.status==='completed'?'<span class="complete-badge">COMPLETED</span>':'<span class="draft-badge">DRAFT</span>'):'';renderCase()}
...
async function loadGroup(i){captureCase();index=Math.max(0,Math.min(summaries.length-1,i));const r=await fetch(`/api/group/${index}?reviewer_id=${encodeURIComponent(reviewer())}`);if(!r.ok){setStatus('Failed to load group',true);return}group=await r.json();$('groupPicker').value=String(index);currentTask=TASKS[0];renderGroup();updateNav();setStatus('')}
function applyFilter(){const mode=$('filter').value;visible=summaries.filter(x=>mode==='all'||(mode==='completed'&&x.status==='completed')||(mode==='pending'&&x.status!=='completed'));const picker=$('groupPicker');picker.innerHTML='';visible.forEach(x=>{const o=document.createElement('option');o.value=x.index;o.textContent=`${String(x.index+1).padStart(3,'0')} | ${x.status.toUpperCase()} | ${x.base_sample_id}`;picker.appendChild(o)});if(!visible.some(x=>x.index===index)&&visible.length)index=visible[0].index;picker.value=String(index);updateNav()}
async function refreshState(load=true){const r=await fetch(`/api/state?reviewer_id=${encodeURIComponent(reviewer())}`);const data=await r.json();summaries=data.groups;$('progress').textContent=`${data.completed} / ${data.total} completed`;applyFilter();if(load&&visible.length)await loadGroup(index)}
